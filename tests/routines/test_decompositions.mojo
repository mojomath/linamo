...
def permute_rows(
    mat: Matrix[Float64], piv: List[Int]
...
def test_lu_2x2() raises:
    ...
    var a = la.matrix[Float64]([[4.0, 3.0], [6.0, 3.0]])
    var result = lu(a)
    ref L = result[0]
    ref U = result[1]
    ref piv = result[2]
...
def test_lu_col_major() raises:
    ...
    var a = la.matrix[Float64]([[2.0, 1.0], [5.0, 3.0]], order="F")
    var result = lu(a)
    ref L = result[0]
    ref U = result[1]
    ref piv = result[2]
...
def test_cholesky_2x2() raises:
    ...
    var a = la.matrix[Float64]([[25.0, 15.0], [15.0, 13.0]])
    var L = cholesky(a)
