...
def test_isclose_within_default_tolerance() raises:
    ...
    var b = la.matrix[DType.float64]([[1.0, 2.000000001], [3.0, 4.0]])
...
def test_isclose_outside_default_tolerance() raises:
    ...
    var a = la.matrix[DType.float64]([[1.0, 2.0]])
    var b = la.matrix[DType.float64]([[1.0, 2.5]])
...
def test_isclose_atol_decides_at_zero() raises:
    ...
    """
    var a = la.matrix[DType.float64]([[1e-9]])
    var b = la.matrix[DType.float64]([[0.0]])
    testing.assert_true(all(isclose(a, b)))
...
def test_isclose_is_asymmetric_in_its_operands() raises:
    ...
    """
    var small = la.matrix[DType.float64]([[1e-8]])
    var zero = la.matrix[DType.float64]([[0.0]])
...
def test_isclose_nan_is_close_to_nothing() raises:
    ...
    var a = la.matrix[DType.float64]([[_nan(), _nan()]])
    var b = la.matrix[DType.float64]([[_nan(), 1.0]])
    var mask = isclose(a, b)
...
def test_isclose_infinities() raises:
    ...
    by equality instead.
    """
    var a = la.matrix[DType.float64]([[_inf(), _inf(), _inf()]])
    var b = la.matrix[DType.float64]([[_inf(), -_inf(), 1e308]])
    var mask = isclose(a, b)
    testing.assert_true(mask[0, 0])
    testing.assert_false(mask[0, 1])
...
def test_isclose_result_is_c_contiguous() raises:
    ...
    var a = la.matrix[DType.float64]([[1.0, 2.0], [3.0, 4.0]], "F")
...
def test_isclose_shape_mismatch_raises() raises:
    ...
    var a = la.matrix[DType.float64]([[1.0, 2.0]])
    var b = la.matrix[DType.float64]([[1.0]])
    var raised = False
    try:
        var _m = isclose(a, b)
    except:
...
def test_allclose_true_and_false() raises:
    ...
    var a = la.matrix[DType.float64]([[1.0, 2.0], [3.0, 4.0]])
    var b = la.matrix[DType.float64]([[1.0, 2.000000001], [3.0, 4.0]])
    var c = la.matrix[DType.float64]([[1.0, 2.0], [3.0, 4.5]])
...
def test_allclose_agrees_with_the_mask() raises:
    ...
    var a = la.matrix[DType.float64]([[1.0, _nan()], [_inf(), 4.0]])
    var b = la.matrix[DType.float64]([[1.0, _nan()], [_inf(), 4.0]])
...
def test_allclose_strided_operands() raises:
    ...
    var a = la.matrix[DType.float64]([[1.0, 9.0], [2.0, 9.0]])
    var b = la.matrix[DType.float64]([[1.0, 0.0], [2.0, 0.0]])
...
def test_scalar_isclose() raises:
    ...
    var m = la.matrix[DType.float64]([[2.0, 2.000000001], [2.5, -2.0]])
...
def test_scalar_isclose_at_zero() raises:
    ...
    var m = la.matrix[DType.float64]([[0.0, 1e-9, 1e-3]])
...
def test_scalar_isclose_strided() raises:
    ...
    var m = la.matrix[DType.float64]([[2.0, 9.0], [2.5, 9.0]])
...
def test_scalar_allclose() raises:
    ...
    var uniform = la.matrix[DType.float64]([[3.0, 3.0], [3.0, 3.0]])
    var mixed = la.matrix[DType.float64]([[3.0, 3.0], [3.0, 3.5]])
...
def test_logical_and_reads_truthiness() raises:
    ...
    """
    var a = la.matrix[DType.float64]([[2.0, 0.0], [2.0, 0.0]])
    var b = la.matrix[DType.float64]([[1.0, 1.0], [0.0, 0.0]])
...
def test_logical_not() raises:
    ...
    var m = la.matrix[DType.float64]([[0.0, 1.0], [-2.0, 0.0]])
...
def test_logical_not_of_a_mask() raises:
    ...
    var m = la.matrix[DType.float64]([[1.0, 5.0]])
...
def test_logical_on_an_integer_dtype() raises:
    ...
    var a = la.matrix[DType.int32]([[2, 0]])
    var b = la.matrix[DType.int32]([[1, 3]])
    var mask = logical_and(a, b)
    testing.assert_true(mask[0, 0])
    testing.assert_false(mask[0, 1])
...
def test_logical_strided_operands() raises:
    ...
    var a = la.matrix[DType.float64]([[1.0, 9.0], [0.0, 9.0]])
    var b = la.matrix[DType.float64]([[1.0, 9.0], [1.0, 9.0]])
...
def test_scalar_logical_forms() raises:
    ...
    var m = la.matrix[DType.float64]([[1.0, 0.0]])
